...
exports.create = async (req, res) => {
	try {
		// fetching the total no. of boards
		const boardsCount = await Board.find().count();

		// creating a new board
		const board = await Board.create({
			user: req.user._id,
			position: boardsCount > 0 ? boardsCount : 1
		});

		// creating default sections within the board
		const toDoSection = await Section.create({
			board: board._id,
			title: 'To Do'
		});

		const doingSection = await Section.create({
			board: board._id,
			title: 'Doing'
		});

		const doneSection = await Section.create({
			board: board._id,
			title: 'Done'
		});

		// adding an empty array of tasks to default sections
		toDoSection._doc.tasks = [];
		doingSection._doc.tasks = [];
		doneSection._doc.tasks = [];
...
exports.getAll = async (req, res) => {
	try {
		// fetching all the boards from the database
